...
- **GitHub Issues에 직접 저장:** 별도 앱 서버나 데이터베이스 없이 Private
  저장소의 이슈를 노트로 사용합니다.
- **정적 SPA:** 브라우저가 GitHub API를 직접 호출합니다. 앱 운영 서버에 노트나
  PAT를 전달하는 중간 단계가 없습니다.
- **태그·검색·휴지통:** GitHub 라벨을 태그로 사용하며 본문 검색과 닫힌 이슈 기반
  휴지통을 지원합니다.
- **파일과 이미지 첨부:** 첨부파일을 같은 저장소에 보관하고 앱과 GitHub 양쪽에서
  확인할 수 있습니다.
- **선택적 종단 간 본문 암호화:** Private 저장소 보호 위에 AES-GCM 암호화를 한 겹
  더 적용할 수 있습니다. 본문 평문과 잠금 숫자는 브라우저 밖으로 보내지 않으며,
  GitHub에는 암호문만 저장합니다. 제목·태그·첨부파일은 암호화 대상이 아닙니다.
...
- PAT와 앱 설정은 사용자의 브라우저에만 보관되며, 인증을 위해 GitHub API로만
  전달됩니다.
- 저장 전 로컬 초안은 해당 브라우저 안에만 남습니다.
...
즉, 정적 파일을 내려받는 일반적인 접속 요청 외에 사용자의 추가 데이터가 GitHub
이외의 앱 운영자나 다른 제3의 서버로 전송되지 않습니다. 실제 노트 데이터가
존재하는 곳은 **본인의 브라우저와 본인이 선택한 GitHub 저장소뿐**입니다.
...
잠금 기능은 별도 환경변수 없이도 공개 기본값으로 동작합니다. 다만 이 기본값은
저장소에 공개되어 있으므로, **이 프로젝트를 포크해 직접 운영하거나 배포한다면 첫
잠금 노트를 만들기 전에 반드시 배포 전용 pepper로 변경하세요.** 값을 한 번
생성해 `.env.local`의 `VITE_NOTE_LOCK_PEPPER`에 설정합니다.

```bash
openssl rand -hex 32
```

생성된 64자리 값을 다음처럼 넣습니다.

```dotenv
VITE_NOTE_LOCK_PEPPER=생성한_64자리_값
```

`.env.local`은 Git에 커밋되지 않습니다. Cloudflare Pages, Workers Builds,
GitHub Actions 등에서 빌드할 때는 같은 이름의 **빌드 환경변수** 또는 secret으로
값을 주입하세요. 외부에서 `npm run build` 후 Workers Static Assets에 올리는
경우에는 빌드한 컴퓨터의 `.env` 또는 `.env.local` 값을 사용합니다. Worker의
런타임 변수는 이미 만들어진 Vite 번들에 적용되지 않습니다.

설정하지 않으면 앱에 포함된 공개 기본값을 사용합니다. 포크 운영자는 생성한 값을
별도로 안전하게 백업하고, 잠금 노트를 만든 뒤에는 모든 웹·데스크톱 빌드에서
동일한 값을 계속 사용해야 합니다. 값을 변경하거나 분실하면 기존 잠금 노트는
6자리 숫자를 알고 있어도 복호화할 수 없습니다.

이 환경변수는 Vite가 정적 JavaScript 번들에 포함하므로 서버 비밀과 같은 보호를
받지는 않습니다. 소스 저장소에 공통값을 하드코딩하지 않고 배포별 암호화 재료를
분리하여, GitHub 이슈 본문만 탈취된 경우에 추가 장벽을 제공하기 위한 값입니다.
공개 호스팅의 빌드 결과에 접근할 수 있는 공격자는 이 값을 분석할 수 있으므로
6자리 잠금 숫자를 고강도 비밀번호나 비밀번호 관리자 대신 사용하지 마세요.
...
`api.github.com`으로 직접 전송됩니다. PAT를 기억하도록 선택하면 브라우저의
`localStorage`에 암호화하지 않은 상태로 보관됩니다. 같은 출처에서 실행되는
스크립트는 이 값에 접근할 수 있으므로 개인 기기에서만 사용하고, 저장소 하나로
범위를 제한한 fine-grained PAT를 사용하세요.
...
일반 노트의 본문 상단 메뉴에서 **잠금**을 선택하면 6자리 숫자로 본문을 암호화할
수 있습니다. GitHub에는 제목 앞의 `🔒`와 Base64로 인코딩된 암호문만 저장됩니다.
잠금 숫자는 저장하지 않고 앱 메모리에서 일정 시간 동안만 재사용합니다. 유지
시간은 기본 1시간이며 설정의 **잠금 숫자 유지 시간**에서 5분부터 8시간까지 고를
수 있습니다. 모든 잠금 노트에 같은 숫자를 사용해야 하며, 숫자나
`VITE_NOTE_LOCK_PEPPER` 중 하나라도 잊으면 내용을 복구할 수 없습니다.

암호키는 6자리 숫자, 배포별 pepper, 이슈 번호와 이슈별 random salt를 결합해
PBKDF2-SHA-256으로 만들고 본문은 AES-GCM으로 암호화합니다. 이슈별 salt와 IV는
복호화에 필요하므로 암호문 바이너리에 포함됩니다. 제목, 라벨과 첨부파일은
암호화 대상이 아닙니다.
...
색상을 사용합니다. 이 표시색은 GitHub에 저장된 색상과 무관합니다. 새 라벨을 만들
때만 계산한 색상을 GitHub에 함께 저장하며, 기존 라벨의 이름을 바꿀 때는 기존
이슈와 색상을 유지하고 이름만 변경합니다.
...
30일이 지난 이슈도 본문과 제목은 저장소에 남습니다. 다만 앱에 연결한 뒤 하루에
한 번, 보존 기간이 지난 닫힌 이슈의 첨부파일과 전용 첨부 댓글은 자동으로
삭제합니다. 이후 이슈를 다시 열어도 본문은 복원되지만 첨부는 복원되지 않습니다.
...
| `src/lib/note-lock.js` | 잠금 제목 규약과 이슈별 본문 암호화·복호화 |
...
처리할 수 있어 SPA가 이 단축키를 안정적으로 가로챌 수 없습니다. Tauri 데스크톱
앱에서는 원래 단축키를 사용할 수 있습니다.

PAT 입력은 password 필드로 표시합니다. 설정 화면을 다시 열 때 저장된 값은 입력란에
채우지 않으며, 새 PAT를 입력한 경우에만 기존 값을 교체합니다. 빈 상태로 설정을
저장하면 기존 PAT를 유지합니다. password 표시는 화면에서 값을 가릴 뿐 저장된 PAT를
암호화하지 않습니다. 자동수정·자동 대문자화는 비활성화하고 붙여넣기에
...
되돌리고, 삭제·복구는 해당 이슈 폴더만 대조해야 합니다. 만료 첨부 정리는
`closed:<YYYY-MM-DD` 조건으로 확인된 이슈의 폴더와 전용 댓글만 삭제해야 합니다.
