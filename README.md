...
- **GitHub Issues에 직접 저장:** 별도 앱 서버나 데이터베이스 없이 Private
  저장소의 이슈를 노트로 사용합니다.
- **가볍고 단순한 GitHub API 래퍼:** 브라우저에서 GitHub API를 직접 호출하므로
  중간 서버에 노트나 PAT를 전달하지 않고, 별도 백엔드에서 생기는 저장·운영
  부담과 공격 표면을 줄였습니다.
- **태그·검색·휴지통:** GitHub 라벨을 태그로 사용하며 본문 검색과 닫힌 이슈 기반
  휴지통을 지원합니다.
- **파일과 이미지 첨부:** 첨부파일을 같은 저장소에 보관하고 앱과 GitHub 양쪽에서
  확인할 수 있습니다.
- **6자리 숫자 본문 잠금:** Private 저장소 보호 위에 AES-GCM 본문 암호화를 한 겹
  더 적용할 수 있습니다. 잠금 숫자는 저장하지 않고 메모리에서 한 시간만
  재사용합니다.
