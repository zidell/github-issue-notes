...
잠금 기능을 사용하려면 배포마다 고유한 pepper를 한 번 생성해 `.env.local`의
`VITE_NOTE_LOCK_PEPPER`에 설정하세요.
...
빌드할 때는 같은 이름의 환경변수 또는 secret으로 값을 주입하세요. 이 값은
배포마다 다르게 만들되, 잠금 노트를 만든 뒤에는 **절대 변경하거나 분실하면 안
됩니다.** 다른 값으로 빌드한 앱에서는 기존 잠금 노트를 복호화할 수 없습니다.
