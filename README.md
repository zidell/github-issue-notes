...
잠금 기능은 별도 환경변수 없이 바로 사용할 수 있습니다. 포크한 앱에서 공개
기본값 대신 배포별 pepper를 사용하려면 값을 한 번 생성해 `.env.local`의
`VITE_NOTE_LOCK_PEPPER`에 선택적으로 설정하세요.
...
`.env.local`은 Git에 커밋되지 않습니다. Cloudflare Pages, GitHub Actions 등에서
빌드할 때는 같은 이름의 환경변수 또는 secret으로 값을 주입할 수 있습니다. 값을
설정하지 않으면 앱에 포함된 공개 기본값을 사용합니다. 배포별 값을 사용한다면
잠금 노트를 만든 뒤에는 같은 값을 계속 유지해야 합니다. 다른 값으로 빌드한
앱에서는 기존 잠금 노트를 복호화할 수 없습니다.
